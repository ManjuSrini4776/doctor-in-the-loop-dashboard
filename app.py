...
def generate_patient_pdf(p, pid, parsed, doc_name, notes, appt_slot, appt_room):
    ...
    def _clean(text):
        """Strip special unicode characters unsupported by Helvetica font."""
        if not text: return ''
        return (str(text)
            .replace('\u2014', '-')   # em dash —
            .replace('\u2013', '-')   # en dash –
    ...
    def body_text(text, indent=0):
        ...
        pdf.set_x(18 + indent)
        pdf.multi_cell(174 - indent, 6, _clean(text))
        pdf.ln(1)
    ...
    def label_value(label, value):
        ...
        pdf.set_font('Helvetica', '', 10)
        pdf.set_text_color(51, 65, 85)
        pdf.cell(0, 7, _clean(str(value)) if value else '-', ln=True)
    ...
    pdf.cell(0, 5, _clean(f'This report was generated by MedAI Clinical System on {now_str} and approved by {doc_name}.'), ln=True, align='C')
    pdf.cell(0, 5, _clean('This is an AI-assisted report reviewed by a qualified doctor. For medical emergencies call 044-2744-0000.'), ln=True, align='C')
...
def build_whatsapp_msg(p, pid, doc_name, appt_slot, appt_room, notes):
    ...
    Respects language selected in the messaging module.
    """
    sev     = p.get('_sev', 'Normal')
    ...
    if lang_code != 'en':
        slot = appt_slot or 'To be scheduled'
        room = appt_room or 'OPD — consult front desk'
        if sev == 'Severe':
            tmpl = MSG_TEMPLATES.get('urgent', {}).get(lang_code) or MSG_TEMPLATES['urgent']['en']
        elif sev == 'Moderate':
            tmpl = MSG_TEMPLATES.get('appointment', {}).get(lang_code) or MSG_TEMPLATES['appointment']['en']
        else:
            tmpl = MSG_TEMPLATES.get('report_ready', {}).get(lang_code) or MSG_TEMPLATES['report_ready']['en']
        msg = tmpl(pid, doc_name, slot, room)
        if notes and notes.strip():
            msg += f"\n\n*Doctor's Instructions:*\n{notes.strip()}"
        msg += (
            f"\n\n📎 *Your full medical report PDF will be attached to this message.*\n"
        ...
        return msg
    # ── English fallback (original logic below) ───────────────
